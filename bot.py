...
async def start(update: Update, context):
    """Comando /start"""
    print(f"Comando /start recebido de {update.effective_user.first_name}")
    await update.message.reply_text("Olá! Eu sou seu bot.")

async def echo(update: Update, context):
    """Responde com a mesma mensagem enviada pelo usuário."""
    print(f"Mensagem recebida: {update.message.text} de {update.effective_user.first_name}")
...
def webhook():
    ...
    try:
        ...
        print(f"Recebido update: {json_update}")  # Loga o update recebido para depuração
        update = Update.de_json(json_update, app.bot)
        app.update_queue.put_nowait(update)
        return "OK", 200
    except Exception as e:
        print(f"Erro no Webhook: {e}")  # Loga erros no processamento do webhook
